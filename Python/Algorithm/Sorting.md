...
- **정렬**이란 *데이터를 특정한 기준에 따라 순서대로 나열*하는 것
...
> N + (N - 1) + (N - 2) + ... + 2
- 이는 (N^2 + N - 2) / 2로 표현할 수 있는데, 빅오 표기법에 따라서 **O(N^2)** 이라고 작성
...
- 특정한 조건이 부합할 때만 사용할 수 있지만 ***매우 빨게 동작하는*** 정렬 알고리즘
...
- 앞서 다른 네 가지 정렬 알고리즘을 비교하면 다음과 같음
- 추가적으로 대부분의 프로그래밍 언어에서 지원하는 *표준 정렬 라이브러리는 최악의 경우에도 **O(NlogN)** 을 보장*하도록 설계
...
end_time = time.time()
# 수행 시간 출력
print("기번 정렬 라이브러리 성능 측정:", end_time - start_time)
