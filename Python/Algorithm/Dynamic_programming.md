...
- 탑다운(메모이제이션) 방식은 **하향식**이라고도 하며 보텁업 방식은 **상향식**이라고도 함
...
```python
# 한 번 계산된 결과를 메모이제이션(Mamoization)하기 위한 리스트 초기화
d = [0] * 100
...
- 다이나믹 프로그래밍과 분할 정복은 모두 **최적 부분 구조**를 가질 때 사용할 수 있음
  - 큰 문제를 작은 문제로 나눌 수 있으며 작은 문제릐 답을 모아서 큰 문제를 해결할 수 있는 상황
...
- 일단 재귀 함수로 비효율적인 완전 탐색 프로그램을 작성한 뒤에 (탑다운) 작은 문제에서 구한 답이 큰 문제에 그대로 사용될 수 있으면, 코드를 개선하는 방법을 사용할 수 있음
