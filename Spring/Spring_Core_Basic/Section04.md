...
> </br>
> 일반적으로 `ApplicationContext`를 스프링 컨테이너라 함

#### 스프링 컨테이너 생성 과정

1. 스프링 컨테이너 생성
![img](../img/spring_core_basic/s4/1.png)
   - `new AnnotationConfigApplicationContext(AppConfig.class)`
   - 스프링 컨테이너를 생성할 때는 구성 정보를 지정해주어야 함
   - 여기서는 `AppConfig.class`를 구성 정보로 지정
2. 스프링 빈 등록
![img](../img/spring_core_basic/s4/2.png)
...
> **주의: 빈 이름은 항상 다른 이름을 부여**해야 함. 같은 이름을 부여하면, 다른 빈이 무시되거나, 기존 빈을 덮어버리거나 설정에 따라 오류가 생김

3. 스프링 빈 의존관계 설정 - 준비
![img](../img/spring_core_basic/s4/3.png)
4. 스프링 빈 의존관계 설정 - 완료
![img](../img/spring_core_basic/s4/4.png)
   - 스프링 컨테이너는 설정 정보를 참고해서 의존관계를 주입(DI)
   - 단순히 자바 코드를 호출하는 것 같지만, 차이가 있음

#### 참고

- 스프링은 빈을 생성하고, 의존관계를 주입라는 단계가 나누어져 있음
- 그런데 이렇게 자바 코드로 스프링 빈을 등록하면 생성자를 호출하면서 의존관계 주입도 한번에 처리
...
- 타입으로 조회시 같은 타입의 스프링 빈이 둘 이상이면 오류가 발생. 이떄는 빈 이름을 지정
...
![img](../img/spring_core_basic/s4/5.png)
```java
package hello.core.beanfind;
...
![img](../img/spring_core_basic/s4/6.png)
...
![img](../img/spring_core_basic/s4/7.png)
- **메세지소스를 활용한 국제화 기능**
...
- 자바 코드, XML, Groovy 등
![img](../img/spring_core_basic/s4/8.png)
...
- 최근에는 스프링 부트를 많이 사용하면서 XML기반의 설정은 잘 사용하지 않음. 아직 많은 레거시 프로젝트 들이 XML로 되어 있고, 또 XML을 사용하면 컴파일 없이 빈 설정 정보를 변경할 수 있는 장점도 있으므로 한번쯤 배워두는것도 괜찮음
- `GenericXmlApplicationContext`를 사용하면 `xml`설정 파일을 넘기면 됨
...
- xml 기반으로 설정하는 것은 최근에 잘 사용하지 않으므로 이정도로 마무리 하고, 필요하면 스프링 공식 레퍼런스 문서를 확인
  - https://spring.io/projects/spring-framework

### 스프링 빈 설정 메타 정보 - BeanDefinition

- 스프링은 어떻게 이런 다양한 설정 형식을 지원하는 것일까? 그 중심에는 `BeanDefinition`이라는 추상화가 있음
...
![img](../img/spring_core_basic/s4/9.png)

#### 코드 레벨로 조금 더 깊이 있게 들어가보자

![img](../img/spring_core_basic/s4/10.png)
- `AnnotaionConfigApplicationContext`는 `AnnotatedBeanDefinitionReader`를 사용해서 `AppConfig.class`를 읽고 `BeanDefinition`을 생성
...
- Constructor arguments, Properties: 의존관계 주입에서 사용함(자바 설정처럼 팩토리 역할의 빈을 사용하면 없음)
```java
...
- BeanDefinition에 대해서는 너무 깊이있게 이해하기 보다, 스프링이 다양한 형태의 설정 정보를 BeanDefinition으로 추상화해서 사용하는 것 정도만 이해하면 됨
- 가끔 스프링 코드나 스프링 관련 오픈 소스의 코드를 볼 때, BeanDefinition 이라는 것이 보일 때가 있음. 이때 이러한 메커니즘을 떠올리면 됨
