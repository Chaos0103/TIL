...
- 객체 지향 프로그래밍은 컴퓨터 프로그램을 명령어의 목록을 보는 시각에서 벗어나 여러개의 독립된 단위, 즉 **"객체"**들의 **모임**으로 파악하고자 하는 것. 각각의 **객체**는 **메시지**를 주고받고, 데이터를 처리할 수 있음 **(협력)**
- 객체 지향 프로그래밍은 프로그램을 **유연**하고 **변경**이 용이하게 만들기 때문에 대규모 소프트웨어 개발에 많이 사용
...
- **DIP**: 의존관계 역적 원칙(Dependency Inversion Principle)
...
#### OCP 개방-폐쇄 원칙(Open/Close Principle) - 문제점
...
- 사용자 인터페이스 -> 운잔자 클라이언트, 정비사 클라이언트로 분리
- 분리하며 정비 인터페이스 자체가 변해도 운전자 클라이언트에 영향을 주지 않음
...
- 앞에서 이야기한 **역할(Role)에 의존하게 해야 한다는 것과 같음**. 객체 세상도 클라이언트가 인터페이스에 의존해야 유연하게 구현체를 변경할 수도 있음!구현체에 의존하게 되면 변경이 아주 어려워짐
