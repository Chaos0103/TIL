...
- 대부분의 스프링 애플리케이션은 웹 애플리케이션. 물론 웹이 아닌 애플리케이션 개발도 얼마든지 개발할 수 있음
- 웹 애플리케이션은 보통 여러 고객이 동시에 요청
![img](../img/spring_core_basic/s5/1.png)
...
```
- 우리가 만들었던 스프링 없는 순수한 DI 컨테이너인 AppConfig는 요청을 할 때 다마 객체를 새로 생성
...
3. 딱 1개의 객체 인스턴스만 존재해야 하므로, 생성자를 private으로 막아서 혹시라도 외부에 new 키워드로 객체 인스턴스가 생성되는 것을 막음
...
> 참고: 싱글톤 패턴을 구현하는 방법은 여러가지가 있음. 여기서는 객체를 미리 생성해두는 가장 단순하고 안전한 방법을 선택
...
- 스프링 컨테이너는 싱글톤 패턴을 적용하지 않아도, 객체 인스턴스를 싱들톤으로 관리
  - 이전에 설명한 컨테이너 생성 과정으 자세히 보자. 컨테이너는 객체를 하나만 생성해서 관리
...
![img](../img/spring_core_basic/s5/2.png)
...
- ThreadA가 사용지A 코드를 호출하고 ThreadB가 사용자B 코드를 호출한다 가정
- `StatefulService`의 `price`필드는 공유되는 필드인데, 특정 클라이언트가 값을 변경
- 사용자A의 주문금액은 10000원이 되어야 하는데, 20000원이라는 결과가 나옴
- 실무에서 이런 경우를 종종 보는데, 이로인해 정말 해결하기 어려운 큰 문제들이 터짐(몇년에 한번씩 꼭 만남)
- 진짜 공유필드는 조심! 스프링 빈은 항상 무상태(stateless)로 설계
...
- orderService 빈을 만드는 코드를 보면 `memberRepository()`를 호출
  - 이 메서드를 호출하면 `new MemoryMemberRepository()`를 호출
...
- 기능 검증을 위해 잠깐 사용하는 것이니 인터페이스에 조회기능까지 추가하지 말 것
...
- AppConfig의 자바 코드를 보면 분명히 각각 2번 `new MemoryMemberRepository`호출해서 다른 인스턴스가 생성되어야 함
...
```
- 스프링 컨테이너가 각각 @Bean을 호출해서 스프링 빈을 생성. 그래서 `memberRepository()`는 다음과 같이 총 4번이 호출되어야 하는 것 아닐까?
  1. 스프링 컨테이너가 스프링 빈에 등록하기 위해 @Bean이 붙어있는 `memberRepository()` 호출
  2. memberService() 로직에서 `memberRepository()` 호출
  3. orderService() 로직에서 `memberRepository()` 호출
- 하지만 출력 결과는 모두 1번만 호출
```
...
- 그런데 예상과는 다르게 클래스 명에 xxxCGLIB가 붙으면서 상당히 복잡해진 것을 볼 수 있음
- 이것은 내가 만든 클래스가 아니라 스프링이 CGLIB라는 바이트코드 조작 라이브러리를 사용해서 AppConfig 클래스를 상속받은 임의의 다른 클래스를 만들고, 그 다른 클래스를 스프링 빈으로 등록한 것

#### 그림

![img](../img/spring_core_basic/s5/3.png)
- 그 임의의 다른 클래스가 바로 싱글톤이 보장되도록 해줌
- 아마도 다음과 같은 바이트 코드를 조작해서 작성되어 있을 것(실제로는 CGLIB의 내부 기술을 사용하는데 매우 복잡)
...
call AppConfig.memberRepository
```
- 이 출력 결과를 통해서 MemberRepository가 총 3번 호출된 것을 알 수 있음
- 1번에 @Bean에 의해 스프링 컨테이너에 등록하기 위해서이고, 2번은 각각 `memberRepository()`를 호출하면서 발생한 코드
...
```
- 당연히 인스턴스가 같은지 테스트 하는 코드도 실패하고, 각각 다 다를 MemoryMemberRepository 인스턴스를 가지고 있음

#### 정리

- @Bean만 사용해도 스프링 빈으로 등록되지만, 싱글톤을 보장하지 않음
  - `memberRepository()`처럼 의존관계 주입이 필요해서 메서드를 직접 호출할 때 싱글톤을 보장하지 않음
- 크게 고민할 것 없음. 스프링 설정 정보는 항상 `@Configuration`을 사용
